...
Accepted (extends [ADR-0001](0001-githog-driven-agent-loop.md) and [ADR-0002](0002-structured-signal-and-resume-toggle.md), supersedes neither)

## Context

ADR-0001 gave githog a per-issue agent loop; ADR-0002 made the completion signal
...
(`/code-review`) and run the tests, then emit `<promise>COMPLETE</promise>`. githog
...
gated by two checks githog itself enforces:

- **A deterministic machine gate.** githog runs a configured `verifyCommand` (the
...
- **A fresh-context adversarial review.** githog spawns a separate `RunReview` pass over
  ...
  `githog-review` skill reads the issue, the task file, and the diff; runs the checks;
