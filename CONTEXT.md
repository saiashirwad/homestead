...
pass), `/homestead-implement` (one iteration), and `/homestead-review` (the fresh-context
...
### Completion handoff
What homestead does when the agent loop ends on the completion sentinel (and, with
...
state. The worktree is left alive for inspection until `homestead kill`. The PR queue is the
...
exists because a builder grading its own exam rationalises its own shortcuts; homestead
...
### Machine gate
The deterministic half of the review-converge cycle: homestead runs a configured
...
machine gate). This is just the project's configured command; homestead adds no formal or
mathematical verification.

### Fresh-context reviewer
The adversarial half of the review-converge cycle: a separate agent invocation
(`/homestead-review`) over the whole diff, run in a **clean context with no shared history
