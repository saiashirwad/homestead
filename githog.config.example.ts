...
export default defineConfig({
  ...
  agent: {
    ...
    loop: {
      ...
      // githog refuses to open a PR on the builder's say-so alone: after a builder
      // `COMPLETE` it runs its own deterministic machine gate, then a fresh-context
      // adversarial reviewer (the seeded githog-review skill, always run with no
      ...
      // reviewSkill: "githog-review", // override the reviewer skill name
