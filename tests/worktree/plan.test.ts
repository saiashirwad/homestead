...
const liveOn =
  (...busy: ReadonlyArray<number>) =>
  (port: number) =>
    Effect.succeed(busy.includes(port))
...
test("pickFreePort skips a port with a live listener and returns the next free one", async () => {
  const picked = await Effect.runPromise(pickFreePort(3000, new Set(), liveOn(3000, 3001), 20))
...
test("pickFreePort records skipped live ports in the used set so the pick is reproducible", async () => {
  ...
  await Effect.runPromise(pickFreePort(3000, used, liveOn(3000, 3001), 20))
  expect(used).toEqual(new Set([3000, 3001]))
})

test("pickFreePort fails after maxAttempts consecutive live ports", async () => {
  const alwaysLive = (_port: number) => Effect.succeed(true)
  const exit = await Effect.runPromiseExit(pickFreePort(3000, new Set(), alwaysLive, 3))
...
test("resolvePortEdits reuses an existing target .env port without probing", async () => {
  ...
  const edits = await Effect.runPromise(resolvePortEdits("PORT=3999\n", ports, used, probe, 20))

  expect(edits).toEqual([["PORT", "3999"]])
...
test("resolvePortEdits skips a port with a live listener even when no sibling .env claims it", async () => {
  ...
  const edits = await Effect.runPromise(resolvePortEdits("", ports, used, liveOn(3000), 20))
...
test("resolvePortEdits keeps two specs from colliding when the first probed port is live", async () => {
  const ports = [
    { key: "WEB_PORT", base: 3000 },
    { key: "API_PORT", base: 3000 },
  ]
  const used = new Map([
    ["WEB_PORT", new Set<number>()],
    ["API_PORT", new Set<number>()],
  ])
