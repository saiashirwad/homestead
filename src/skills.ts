...
- If every task and criterion is now checked, do a cheap first-pass self-review of the
  ...
  the project opts into review-converge (ADR-0003), githog runs its own deterministic
...
const reviewBody = (loop: ResolvedLoop): string =>
  `You are running githog's **adversarial review pass** for a GitHub issue (URL given
...
- **Found real defects** → append them to \`${loop.taskFile}\` as new **vertical-slice
  ...
  Do NOT commit \`${loop.taskFile}\` (githog git-ignores it). Then emit the findings
  signal exactly: \`${loop.sentinels.reviewFindings}\`. githog will run another build
  iteration to fix them.
- **Diff is genuinely clean** (every criterion met, no real defect) → emit the clean
  signal exactly: \`${loop.sentinels.reviewClean}\`. githog will open the PR.
...
const reviewDoc = (loop: ResolvedLoop, name: string): string =>
  frontmatter(name, "githog review pass: fresh-context adversarial review of the diff against the spec; append fix tasks or sign off clean", reviewBody(loop));
