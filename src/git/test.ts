...
export class GitTestHandle extends Context.Service<GitTestHandle, GitTestApi>()(
  "homestead/git/test/GitTestHandle",
) {}
...
const buildGitTest = Effect.gen(function* () {
  ...
  const handle: GitTestApi = {
    ...
    setStatus: (cwd, porcelain) => Ref.update(statuses, (m) => new Map(m).set(cwd, porcelain)),
    setWorktrees: (cwd, entries) => Ref.update(worktreesByCwd, (m) => new Map(m).set(cwd, entries)),
  ...
  const git: typeof Git.Service = {
    ...
    refExists: (cwd, ref) =>
      Ref.get(refExistsMap).pipe(Effect.map((m) => m.get(key(cwd, ref)) ?? false)),
    symbolicRef: (cwd, name) =>
      Ref.get(symbolicRefs).pipe(Effect.map((m) => m.get(key(cwd, name)))),
    merge: (cwd, branch) =>
      Effect.gen(function* () {
    ...
    stash: {
      push: (cwd, _message) =>
        Ref.update(journal, (j) => ({ ...j, stashPushes: [...j.stashPushes, cwd] })).pipe(
          Effect.as(true),
        ),
      pop: (cwd) =>
        Ref.update(journal, (j) => ({ ...j, stashPops: [...j.stashPops, cwd] })).pipe(
          Effect.as(true),
        ),
    },
    worktree: {
      list: (cwd) => Ref.get(worktreesByCwd).pipe(Effect.map((m) => m.get(cwd) ?? [])),
      ...
      prune: (cwd) => Ref.update(journal, (j) => ({ ...j, prunes: [...j.prunes, cwd] })),
    },
    branch: {
      ...
      listLocal: (cwd) => Ref.get(localBranches).pipe(Effect.map((m) => m.get(cwd) ?? [])),
    },
...
export const GitTest = Layer.effectContext(
  buildGitTest.pipe(
    Effect.map(({ git, handle }) =>
      Context.make(Git, git).pipe(Context.add(GitTestHandle, handle)),
    ),
  ),
)
