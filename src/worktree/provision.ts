...
import type { HomesteadConfig, Plan } from "../types.ts"
...
export const ensureServices = Effect.fnUntraced(function* (repo: Repo, config: HomesteadConfig) {
  for (const service of config.services ?? []) {
    ...
    if (start === undefined || start.length === 0) {
      return yield* ServiceUnavailable.make({
        name: service.name,
        host: service.host,
        port: service.port,
