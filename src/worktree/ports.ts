...
export const readReservations = Effect.fn("homestead/read-reservations")(function* (
  repoName: string,
) {
...
const acquireLock = Effect.fn("homestead/acquire-provision-lock")(function* (repoName: string) {
  ...
  for (let attempt = 0; attempt < LOCK_MAX_ATTEMPTS; attempt++) {
    const got = yield* fs.writeFileString(file, `${process.pid}\n`, { flag: "wx" }).pipe(
      Effect.as(true),
...
export class PortAllocator extends Context.Service<PortAllocator>()(
  "homestead/worktree/ports/PortAllocator",
  {
