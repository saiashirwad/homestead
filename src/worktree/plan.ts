import { Clock, Console, DateTime, Effect, FileSystem, Path } from "effect"
import * as os from "node:os"
import { Git } from "../git/service.ts"
import { nextFreePort, readEnvVar } from "../text.ts"
import { probeTcp } from "../process.ts"
import { DEFAULT_ENV_FALLBACK, DEFAULT_ENV_SOURCE } from "../defaults.ts"
import type { HomesteadConfig, Plan, PortSpec, WorktreeContext } from "../types.ts"
import {
  liveReservations,
  readReservations,
  reservationsToClaim,
...
export const resolvePlan = Effect.fnUntraced(function* (
  ...
  probe: (port: number) => Effect.Effect<boolean> = (port) =>
    probeTcp(PROBE_HOST, port, PROBE_TIMEOUT_MS),
) {
  const git = yield* Git
    ...
    ports.length === 0
      ...
      : yield* withRegistryLock(
          ...
          Effect.gen(function* () {
            ...
            const reserved = liveReservations(yield* readReservations(repo.repoName), now)
            ...
            const claims = reservationsToClaim(
              ...
              target.branch,
              process.pid,
              DateTime.formatIso(currentDt),
            )
            if (claims.length > 0) {
              const survivors = reserved.filter(
                (r) => !(r.branch === target.branch && r.pid === process.pid),
              )
